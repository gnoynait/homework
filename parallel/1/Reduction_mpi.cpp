...
int main (int argc, char* argv[]){
	...
	if (world_rank == 0) {
		...
		double *buffer = new double[split_size * M]();
		...
		buffer[3] = -2;
		int next = 0;
		MPI_Recv(buffer, split_size * M, MPI_DOUBLE, 0, tag, MPI_COMM_WORLD, &Stat);
		/*
		cout << "receive" << endl;
		for (int i = 0; i < split_size * M; ++i) {
			cout << buffer[i] << " ";
	...
	if (world_rank == 0) {
		double *buffer = new double[M * M];
		for (int proc = 1; proc < world_size; ++proc) {
			MPI_Recv(buffer, M * M, MPI_DOUBLE, proc, tag, MPI_COMM_WORLD, &Stat);
			for (int i = 0; i < M; ++i) {
				for (int j = 0; j < M; ++j) {
